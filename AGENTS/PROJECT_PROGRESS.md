...
| D5 | 视频预览 + 导出 | ✅ | 后端提供导出URL，前端 `VideoPreview` 和 `TaskDetail` 支持预览及下载 |

### 模块 E：用户体验（P0）

| # | 任务 | 状态 | 说明 |
|---|------|:----:|------|
| E1 | 任务进度展示（阶段状态 + 进度条） | ✅ | 后端 `TasksGateway` WebSocket 发送进度，前端 `useTaskSubscription` hook 渲染进度条 |
| E2 | 失败兜底：错误分类 + 友好提示 + 一键重试 | ✅ | 结构化 HttpExceptionFilter，前端提供重试按钮与明确错误状态展示 |
...
| # | 任务 | 状态 | 说明 |
|---|------|:----:|------|
| D6 | 智能剪辑：分镜拼接 + 转场 + 字幕（FFmpeg） | ⬜ | 无 FFmpeg 集成，无本地合成 |
...
| # | 任务 | 状态 | 说明 |
|---|------|:----:|------|
| E4 | 响应式布局（桌面优先 + 移动端可查看） | ✅ | Ant Design 栅格完整支持响应式（useMediaQuery未实际消费） |
...
**整体进度更新：大幅领先预期，已完成 27 项，部分完成 8 项，未开始 11 项（58% 核心完成度）。核心的生成与剧本工作流已经打通并实现前端。**
