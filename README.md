...
5. **分镜视频生成与按需导出**：以每个分镜的 `visual_prompt` 为核心生成独立视频片段，使用上一段尾帧作为下一段首帧保持连续性；生成阶段默认保留分段预览，用户点击“导出完整视频”时再通过 FFmpeg 拼接完整 MP4。
...
- 使用大模型进行素材的多模态理解分析，并根据商品特征与灵感模板，自动输出 JSON Schema 格式的结构化视频脚本与分镜信息。
- 视频生成阶段采用分镜优先 prompt：优先使用 `scene.visual_prompt`，仅补充必要商品身份、镜头、旁白、字幕和连续性约束，避免全局叙事信息稀释单个分镜效果；任务进度按分镜展示并支持失败后从失败镜头继续。

### 智能剪辑与视频合成
- 后端按分镜逐段生成视频，并通过 `return_last_frame` 获取尾帧；后续分镜将上一段尾帧作为首帧输入。生成任务完成时只保存分段视频，默认预览第一段并保留分段切换入口。
- 完整 MP4 只在用户点击“导出完整视频”时按需拼接，输出到 `packages/backend/uploads/generated/{taskId}.mp4`，并通过 `/uploads/generated/{taskId}.mp4` 提供访问；如果 FFmpeg 拼接失败，任务状态和分段预览不受影响，页面会给出清晰导出失败提示。
...
- **项目完成度**：目前已处于 **可用 Demo 阶段**（已打通 58% 核心任务）。工程基础设施、任务调度队列和 WebSocket 骨架已完全建立；素材模块与大模型剧本生成管线已闭环。详细进度见 `AGENTS/PROJECT_PROGRESS.md`。
- **项目亮点 / 创新点**：
  1. TODO: 填写亮点1
  2. TODO: 填写亮点2
  3. TODO: 填写亮点3
