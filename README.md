...
1. **素材与参考视频管理**：支持本地素材上传、筛选、预览、删除，以及素材多模态分析的基础流程。支持原爆款参考视频的本地上传和多模态结构化异步拆解（包含 Hook 提取）。
2. **脚本与灵感工作台**：提供支持素材关联的多种脚本生成入口、支持【爆款仿写】生成模式（可选择原爆款视频并提取拆解上下文进行 1:1 结构仿写）、面向用户的灵感模板广场（支持填表快速生成带货视频方案）、后台模板管理、结构化分镜输出与包含双栏对比的脚本编辑能力，并支持在“我的作品”中集中管理生成结果。
...
## 2026-06-08 爆款仿写与参考视频分析更新

- **参考视频管理 (C1)**：新增【参考视频库】，支持本地上传原爆款视频。上传后，后端会自动将任务推入 BullMQ 队列，使用火山引擎 File API 将视频上传至远端大模型并调用 `VOLCANO_TEXT_*` 种子模型进行多模态拆解。模型输出结构化的 JSON 拆解报告，包含原爆款的 Hook 设计、镜头节奏和视听语言特征。
- **爆款仿写模式 (C5)**：在【生成剧本】的创作入口中，新增“爆款仿写生成”模式。用户可从素材库中选择自己的商品素材，并选择一个已拆解完成的参考视频。后端在生成剧本时，会将商品素材与原爆款的拆解报告合并为上下文，引导 AI 进行 1:1 结构仿写，保留原爆款的吸引力与转化逻辑。
- **双栏对比视图**：剧本编辑器新增双栏对比模式，左栏可直观展示原爆款的解析概况（如来源平台、分类及核心 Hook），方便用户对比修改新生成的剧本内容。
