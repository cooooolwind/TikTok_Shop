...
5. **分段视频自动合成**：使用 FFmpeg 将火山引擎返回的多段视频拼接为完整 MP4，预览页默认播放完整成片并保留分段查看入口。
...
### 智能剪辑与视频合成
- 后端生成多段视频后，会使用 FFmpeg 自动下载并拼接分段，输出完整 MP4 到 `packages/backend/uploads/generated/{taskId}.mp4`，并通过 `/uploads/generated/{taskId}.mp4` 提供访问；旧任务在导出完整视频时会自动补拼接，前端预览页默认播放完整视频并保留分段预览入口。
