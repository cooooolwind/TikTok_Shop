...
## 2026-06-06 灵感模板市场演示更新

- 新增用户侧 `灵感模板广场`，路径为 `/template-market`，以卡片形式展示启用状态的带货视频模板，包含模板名称、策略、适用类目、因子数、更新时间和“使用该模板”入口。
- 新增模板使用页 `/template-market/:id/use`，用户填写商品名称、类目、卖点、价格、目标人群、优惠信息、视频时长和期望风格后，可生成带货视频方案。
- 生成结果包含视频标题、口播脚本、5 个分镜、每个分镜的视频生成提示词、发布文案和话题标签；当前使用后端 mock 生成，保证比赛演示流程可完整跑通。
- 新增 `我的作品` 页面 `/my-videos`，支持将生成结果保存后查看商品名称、使用模板、生成时间、状态和详情。
- 后台 `灵感模板管理` 保留表格维护方式，并补充模板因子、提示词和启用/停用状态配置；内置模板为只读，自定义模板可新增、编辑、删除。
- 后端新增 `/api/v1/templates/:id/generate` 和 `/api/v1/my-videos`；同时为模板接口提供 `/api/v1/inspiration-templates` 别名，便于对外演示。

## 2026-06-06 脚本生成素材选择更新

- 新建剧本页的四种入口都支持从素材库选择图片或视频素材。
- 选择素材生成、剧本模板生成、粘贴文本解析会把所选素材 ID 一起提交到脚本生成任务。
- 自己写剧本会把所选素材保存到草稿的 `source_material_ids`，便于后续查看和继续生成。
