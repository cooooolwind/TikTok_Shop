...
- 视频生成阶段采用“商品图 + 分镜首帧”策略：每个分镜优先调用 Seedream 生成商品锚定首帧，再用 Seedance 生成该分镜视频；如果首帧生成失败，则使用商品图作为兜底输入继续生成，不退化为纯文本视频。

### 智能剪辑与视频合成
- 后端按分镜逐段生成视频，并为每个分镜单独生成商品锚定首帧；生成任务完成时只保存分段视频，默认预览第一段并保留分段切换入口。商品图是视频生成强约束：若缺少商品图，后端会返回明确错误并提示先上传或填写商品图。
