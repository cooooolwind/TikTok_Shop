...
5. **分镜视频生成与自动合成**：以每个分镜的 `visual_prompt` 为核心生成独立视频片段，使用上一段尾帧作为下一段首帧保持连续性，并通过 FFmpeg 拼接为完整 MP4；预览页默认播放完整成片并保留分段查看入口。
...
- 视频生成阶段采用分镜优先 prompt：优先使用 `scene.visual_prompt`，仅补充必要商品身份、镜头、旁白、字幕和连续性约束，避免全局叙事信息稀释单个分镜效果。

### 智能剪辑与视频合成
- 后端按分镜逐段生成视频，并通过 `return_last_frame` 获取尾帧；后续分镜将上一段尾帧作为首帧输入，最后使用 FFmpeg 自动下载并拼接分段，输出完整 MP4 到 `packages/backend/uploads/generated/{taskId}.mp4`，并通过 `/uploads/generated/{taskId}.mp4` 提供访问；旧任务在导出完整视频时会自动补拼接，前端预览页默认播放完整视频并保留分段预览入口。
- 如果 FFmpeg 拼接失败，任务仍会保留已生成的分段结果，页面可继续预览分段视频，避免用户丢失生成成果。
