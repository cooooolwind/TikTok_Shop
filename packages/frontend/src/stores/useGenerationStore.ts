...
export const useCreationStore = create<CreationState>((set, get) => ({
  ...
  createVideo: async (params) => {
    set({ creating: true });
    try {
      const task = await generationApi.create(params);
      set({ creating: false });
  ...
  quickGenerate: async (params) => {
    set({ creating: true });
    try {
      const task = await generationApi.quickCreate(params);
      set({ creating: false });
  ...
  retry: async (taskId) => {
    try {
      const task = await generationApi.retry(taskId);
      set((s) => ({
  ...
  cancel: async (taskId) => {
    try {
      const task = await generationApi.cancel(taskId);
      set((s) => ({
        tasks: s.tasks.map((t) => (t.id === taskId ? task : t)),
        currentTask: s.currentTask?.id === taskId ? task : s.currentTask,
      }));
  ...
  exportVideo: async (taskId, format, resolution, quality) => {
    const result = await generationApi.export(taskId, { format: format as 'mp4' | 'webm', resolution: resolution as '1080x1920' | '1920x1080' | '720x1280', quality: quality as 'high' | 'medium' | 'low' });
    return result;
  },

  regenerateSceneVideo: async (taskId, sceneId, instruction, materialId) => {
    try {
      const task = await generationApi.regenerateScene(taskId, sceneId, {
        instruction,
